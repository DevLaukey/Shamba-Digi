...
router.get("/addinfo", isValidUser, async function (req, res, next) {
  User.findOne({ _id: req.user._id })
    .then((result) => {
      return res.render("addinfo", { "accounts": result });
    })
    .catch((err) => {
...
async function addItemToDatabase(req, res) {
  ...
  try {
    ...
    User.findOne({ _id: req.body.id }, function (err, user) {
      ...
      return res.render("accounts", { error: error, user: user });
    });
  }
}

// view accounts
router.get("/account", isValidUser, async function (req, res, next) {
  User.findOne({ _id: req.user._id })
    .then((user) => {
    Account.find({ userid: user._id })
        .then((items) => {
          return res.render("account", { accounts: items, user: user });
        })
        .catch((err) => {
          //return res.status(501).json(err);
          res.redirect("/users/dashboard");
...
router.get("/editaccount/:id", isValidUser, async function (req, res, next) {
  id = req.params.id;
  User.findOne({ _id: req.user._id })
    .then((user) => {
      Account.findOne({ _id: id })
        .then((item) => {
          return res.render("edititem", { account: item, user: user });
        })
        .catch((err) => {
          //return res.status(501).json(err);
          res.redirect("/users/dashboard");
...
router.post('/edititem/:id',isValidUser, async function(req,res,next){
  ...
  Account.update({_id:id},{$set:{cost:cost,income:income,profit:profit,date:date}}).then(item=>{
    return res.redirect('/accounts/edititem/'+id)
    }).catch(err=>{
      //return res.status(501).json(err);
      res.redirect("/users/dashboard")
    })
